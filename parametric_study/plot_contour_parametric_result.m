...
PlotOptionDef.nLvl.value  = '11'; %'[0:0.2:0.8,0.9,1.0]';
PlotOptionDef.nLvl.data  = 'num';

PlotOptionDef.nLvlTotalRange.desc   = 'lvls full range';
PlotOptionDef.nLvlTotalRange.type   = 'checkbox';
PlotOptionDef.nLvlTotalRange.value  = true;
PlotOptionDef.nLvlTotalRange.data   = 'num';

PlotOptionDef.zMinMaxRange.desc   = 'data range';
PlotOptionDef.zMinMaxRange.type   = 'text';
PlotOptionDef.zMinMaxRange.value  = '[-inf, inf]'; %'[0, 1]';
...
PlotOptionDef.colorbarTick.desc   = 'prefix on tick';
PlotOptionDef.colorbarTick.type   = 'checkbox';
PlotOptionDef.colorbarTick.value  = true;
PlotOptionDef.colorbarTick.data  = 'num';
...
zDataFullND = eval(tmpStr); % Compute full-ND matrix given z-expression

% Find global max and min:
[zDataGlobalMin, zDataGlobalMinIdx] = min(zDataFullND(:));
[zDataGlobalMax, zDataGlobalMaxIdx] = max(zDataFullND(:));
zDataCompleteRange = [zDataGlobalMin, zDataGlobalMax];

tmpIdx = cell(1,length(size(zDataFullND)));
[tmpIdx{:}] = ind2sub(size(zDataFullND),zDataGlobalMaxIdx);
strMax = sprintf('max = %.3g at %s',zDataGlobalMax,...
   strrep(mat2str(cat(2,tmpIdx{:})),' ',','));
tmpIdx = cell(1,length(size(zDataFullND)));
[tmpIdx{:}] = ind2sub(size(zDataFullND),zDataGlobalMinIdx);
strMin = sprintf('min = %.3g at %s',zDataGlobalMin,...
   strrep(mat2str(cat(2,tmpIdx{:})),' ',','));
...
uicontrol( ...
   'Style',       'edit',...
   'String',      sprintf('Global maximum and minimum value\n%s\n%s',strMin,strMax),...
   'Min',         0,...
   'Max',         2,...
   'Enable',      'inactive',...
   'FontName',    'times', ...
...
PlotOption.zMinMaxRange(PlotOption.zMinMaxRange == -inf) = zDataCompleteRange(1);
PlotOption.zMinMaxRange(PlotOption.zMinMaxRange == +inf) = zDataCompleteRange(2);
...
PlotUserData.zDataCompleteRange = zDataCompleteRange;
...
zDataRange  = [min(zData(:)), max(zData(:))];
zDataCompleteRange = [min(tmpUserData.zDataFullND(:)),...
   max(tmpUserData.zDataFullND(:))];

if PlotOption.nLvlTotalRange
   if ~isempty(PlotOption.zMinMaxRange)
      useDataRange = PlotOption.zMinMaxRange;
   else
      useDataRange = zDataCompleteRange;
   end
else
   useDataRange = zDataRange;
end
...
if PlotOption.cfill
   [C,h] = contourf(xData,yData,funDoLog(zData),...
...
if PlotOption.colorbar
   if PlotOption.zLogScale
      ...
      if PlotOption.colorbarTick
         colorbar('Ticks',cLvlFix,'TickLabels',arrayfun(@(lvl) num2sci(lvl,3),cLvl,'un',0));
      else
         colorbar('Ticks',cLvlFix,'TickLabels',arrayfun(@(lvl) sprintf('%.3g',lvl),cLvl,'un',0));
      end
   end
end
...
% Set maximum color range:
if ~isempty(PlotOption.zMinMaxRange)
   caxis(funDoLog(PlotOption.zMinMaxRange))
else
   caxis(funDoLog(zDataCompleteRange))
end
